  ...
  const processMessage = async (message: string) => {
    try {
      const detectResponse = await axios.post(
        "https://translation.googleapis.com/language/translate/v2/detect?key=AIzaSyBqMWKRONW_qUJOnAksH7AuX6uYs7no-EY",
        { q: message }
      );
  ...
  const handleTranslate = async (messageId: string, targetLanguage: string) => {
    ...
    try {
      const response = await axios.post(
        "https://translation.googleapis.com/language/translate/v2?key=AIzaSyBqMWKRONW_qUJOnAksH7AuX6uYs7no-EY",
        {
